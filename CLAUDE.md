...
| ไฟล์ | หน้าที่ | บรรทัด |
|------|--------|--------|
| [content.js](content.js) | Logic ทั้งหมด: fetch hook, scan, print, UI, planning, address book | ~12,500 |
| [content.css](content.css) | Style: widget, modal, plan panel, cards, address book | ~4,160 |
...
| Picking List PDF | `buildPickingListPages()` | optional, per chunk; แสดง calendar mode เมื่อ filter active |
| Multi-page label (2+ SKU) | `splitIdsIntoApiSegments()` → `pagesPerId[]` | per-ID API call สำหรับ multi-SKU; alias/divider map ถูกหน้า |
| Buyer/seller notes | `overlayAliasOnPdf()` + `buildNoteZoneDividerPage()` | `ล★N` ซ้ายบน (ลูกค้า) + `ร★N` ต่อข้างๆ (ร้านค้า) — index แยก, safe glyphs (★+ไทย) |
...
| **Address book (pre-print capture)** | `captureAddressFromOrderGet()` · `tryBackfillAddresses()` · `openAddressBookModal()` | จับ recipient address ก่อน TTS → IndexedDB `qf_address_book` · toggle on/off · CSV export · search |
...
| `qf_last_divider_v1` | fast-path divider toggle 'true'/'false' | 5 bytes |
...
| `qf_address_capture_v1` | address-capture toggle 'true'/'false' | 5 bytes |
...
| `qf_address_book` | `addresses` (keyPath: `orderId`) | `{orderId, fulfillUnitId, recipientName, recipientPhone, addressDetail, subDistrict, district, province, zipcode, countryCode, carrier, productNames, capturedAt, source, isMasked}` | auto-drop >90 วัน ตอน save · ไม่ overwrite unmasked ด้วย masked |
...
**Multi-page label mapping (ออเดอร์ 2+ SKU):**
- TikTok render ฉลาก 1 ใบเป็น 2+ หน้าเมื่อ SKU เยอะหรือชื่อยาว → page-to-id mapping แบบ `round(pages/ids)` พัง (alias ล็อคผิดหน้า, divider หยิบหน้าผิด)
- `splitIdsIntoApiSegments(ids)` แบ่ง ids:
  - multi-SKU id (`skuList.length > 1`) → segment ละ 1 ID (รู้ page count แน่นอน)
  - single-SKU ต่อเนื่อง → merge เข้าด้วยกันจนถึง PRINT_BATCH_SIZE
- แต่ละ segment ยิง generate API แยก → `deriveSegmentPagesPerId()` คืน `pagesPerId[]`
- `pagesPerId[]` ส่งต่อ: `buildChunkPdf` → `overlayAliasOnPdf` + `prependDividersToChunk`; `callGenerateApiRaw` → `buildMultiSkuCombinedPdf` (แก้ note/normal page-range ด้วย)
- Single-SKU ที่ return 2 หน้าผิดคาด → fallback distribute extras ให้ leading ids (rare, best-effort)
...
### Address book — pre-print capture flow

**Why:** หลัง print+TTS ทุก `/order/get` response จะถูก mask (`button_status:3` + `mask_infos`) — unmasked address หาได้แค่ช่วงก่อนพิมพ์ครั้งแรก หรือใน TikTok UI ต้องกด 👁 eye icon ทีละ field

**Primary unmask endpoint — `/api/fulfillment/orders/buyer_contact_info/get`** (fired on eye-icon click):

```
Request:   { main_order_id, contact_info_type }
Response:  { code: 0, data: { plain_text_* } }
```

| contact_info_type | Response field | ตัวอย่างค่า |
|---|---|---|
| 0 | `plain_text_name` | "ธัญลักษณ์ สบกระโทก" |
| 1 | `plain_text_address` | `{items[], region, districts[]}` |
| 2 | `plain_text_phone_number` | "(+66)659549268" |
| 3 | `plain_text_nickname` | "toeystory.tt" |

ส่ง 4 calls ต่อ order เพื่อเก็บครบทุก field → merge ลง record เดียวผ่าน `mergeAddressPatch(orderId, patch)`

**Three capture paths** (ทั้งหมด → IndexedDB `qf_address_book`, keyed by `orderId`):
1. **Passive eye-click sniff** — user กด 👁 บน order detail page → fetch hook จับ response → `captureBuyerContactFieldResponse(reqBody, respJson)` → `mergeAddressPatch(...)`
2. **Passive order-get sniff** — fallback เดิม: `captureAddressFromOrderGet(json)` → `parseOrderDetail(od)` → `saveAddressBatch([...])` (ก่อน mask)
3. **Active backfill** — `buildChunkPdf` เรียก `await tryBackfillAddresses(batch)` ก่อน generate API
   - **ใช้ buyer_contact_info เป็นหลัก** (ส่ง 4 calls ต่อ order — name/address/phone; ข้าม nickname)
   - Fallback เป็น `/order/get` ถ้ายังไม่เคย sniff template buyer_contact_info

**Rate limit — TikTok ใช้ "soft reject" per-account per-day quota (ไม่ใช่ HTTP 429):**

ผ่าน CDP test ยิง ~3,775 calls (sequential + parallel + sustained 1000-call batches) ไม่เจอ HTTP 429 / code 50002 เลย. TikTok ใช้ **quota รายวัน** สำหรับ PII reveals (type 0/1/2). พอ quota หมด:

```json
// Quota exhausted response (looks like success!):
{
  "code": 0, "message": "success",
  "data": {
    "contact_customer_service": true,
    "rejection_message": "เพื่อปกป้องความเป็นส่วนตัวของลูกค้า..."
  }
}
```

- Type 3 (nickname) **ไม่โดน quota** — public info, เรียกได้ตลอด
- Type 0 (name), 1 (address), 2 (phone) → PII-protected, quota burns per call (1 field = 1 call)
- Quota reset ~24h (ยังไม่ verify exact)
- QPS ไม่มี (ทดสอบ 300+ rps sustained ไม่โดน throttle)

**Adaptive circuit-breaker:**
- เริ่ม conservative: concurrency 1, gap 600ms (defaults)
- Exponential backoff on hit: 2s → 4s → 8s → ... cap 5 min
- Trip ถาวรหลัง 3 consecutive hits → `showToast('ดึงข้อมูลเกินโควต้ารายวัน → reset พรุ่งนี้')`
- Session budget cap: `ADDRESS_BACKFILL_MAX_PER_SESSION = 400` calls
- Success ลด gap ลง 25% (floor 400ms)

**Hit detection (`isRateLimitResponse`):**
- **Soft reject (primary):** `code: 0 && data.contact_customer_service === true` ← TikTok's actual signal
- HTTP 429 / `respJson.code === 429` (fallback)
- `respJson.code === 50002 | 50005` (fallback, ยังไม่เคยเจอ endpoint นี้)
- message contains: `rate limit | too many | frequently | request limit | qps | throttle | 429`

**Inspection helpers (DevTools):**
```javascript
window.__qfRateLimitState()           // {consecutiveHits, tripped, backoffUntil, currentGapMs, backfillCount}
window.__qfBuyerContactTemplate()     // captured signed URL + body — ต้อง click eye ก่อนถึง prime
window.__qfProbeContactRateLimit('orderId', { burst: 20, gapMs: 100 })  // sequential probe
window.__qfProbeParallelContact('orderId', { burst: 50, type: 3 })      // parallel burst
```

**Safety:**
- `mergeAddressPatch` immutable merge — patch ใหม่ wins per field, `isMasked:false` sticks
- `saveAddressBatch` ไม่ overwrite unmasked ด้วย masked
- ไม่ block print path: `try { await tryBackfillAddresses } catch {}` — fail silently
- Toggle `state.addressCaptureEnabled` (default ON)
- 90-day auto-purge ตอน save

**Entry points:**
- UI: 📇 button ใน header (ข้าง ⏱ history) → `openAddressBookModal()`
- Badge แสดงจำนวน unmasked records
- CSV export พร้อม BOM สำหรับ Excel Thai
...
| ค่า | ความหมาย | constant | segment |
|-----|---------|---------|---------|
| 30 | ยังไม่พิมพ์ | `LABEL_STATUS_NOT_PRINTED` | `not_printed` |
| 40 | การพิมพ์ไม่สำเร็จ | `LABEL_STATUS_PRINT_FAILED` | `failed` |
| 50 | พิมพ์แล้ว | `LABEL_STATUS_PRINTED` | `printed` |

Segment filter ตอนนี้มี 4 ตัวเลือก: `not_printed` / `failed` / `printed` / `all` — แต่ละ segment ส่ง server query `['<code>']` ตรง ๆ ไม่ทับซ้อน · `isLabelsDone()` ยกเว้นทั้ง `printed` และ `failed` (tab สำหรับ reprint)
...
| `/api/fulfillment/orders/buyer_contact_info/get` | POST | **Unmask eye-click** (1 call/field) | `main_order_id` + `contact_info_type` 0..3 — **rate-limited, exact threshold unknown** |
