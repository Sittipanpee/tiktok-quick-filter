...
| ไฟล์ | หน้าที่ | บรรทัด |
|------|--------|--------|
| [content.js](content.js) | Logic ทั้งหมด: fetch hook, scan, print, UI, planning, note backfill | ~12,500 |
| [content.css](content.css) | Style: widget, modal, plan panel, cards | ~4,160 |
...
| Multi-SKU divider (ออเดอร์แปลก) | `renderMultiSkuDivider()` | hybrid layout (1/2/3-col); group โดย SKU set fingerprint; แสดงทุก SKU + รูปในใบคั่นเดียว; overflow ได้ |
...
| Pre-print note backfill | `tryBackfillNotes()` → `tryBackfillNotesViaOrderGet()` | ดึง buyer/seller remark จาก `/order/get` ก่อน generate API · rate-limit circuit breaker |
...
### Pre-print note backfill flow

**Why:** Labels list API exposes `has_*_note` flags ที่ไม่ trustworthy — buyer/seller remark text มีแต่ใน `/order/get` response. ก่อน print divider ต้อง fetch note text สำหรับ records ที่ยังไม่เคย sniff.

**Two capture paths** → patch `state.records[*].buyerNote / sellerNote` (immutable copy):

1. **Passive `/order/get` sniff** — fetch hook clones every `/order/get` response → `captureNotesFromOrderGetResponse(json)` → walks `main_order[]` → `captureNotesFromOrderDetail(od)`. Field paths: `od.buyer_remark.note | od.buyer_message`, `od.seller_remark.note`.
2. **Active backfill** — `buildChunkPdf` เรียก `await tryBackfillNotes(batch)` ก่อน generate API. ส่ง `/order/get` 1 call ต่อ orderId ที่ยังไม่มี note text. Dedupe ผ่าน `seen` set.

**Template prime:** `/order/get` URL + body ต้อง capture สดผ่าน fetch hook ก่อน (TikTok request signing เป็น path-specific — replays return code 21001001). Module bootstrap จะ inject hidden iframe ที่ `/order/detail?order_no=…` แล้ว postMessage URL+body กลับ → `primeOrderGetTemplate()`.

**Adaptive circuit-breaker (`RATE_LIMIT_STATE`):**
- เริ่ม conservative: `currentConcurrency = 1`, `currentGapMs = 600ms` (defaults)
- Exponential backoff on hit: 2s → 4s → 8s → ... cap 5 min
- Trip ถาวรหลัง 3 consecutive hits → `showToast('ดึงหมายเหตุออเดอร์เกินโควต้ารายวัน → reset พรุ่งนี้')`
- Session budget cap: `NOTE_BACKFILL_MAX_PER_SESSION = 400` calls (`_noteBackfillCount` tracks usage)
...
- `respJson.code === 50002 | 50005` (fallback)
- message contains: `rate limit | too many | frequently | request limit | qps | throttle | 429`

**Inspection helpers (DevTools):**
```javascript
window.__qfRateLimitState()                  // {consecutiveHits, tripped, backoffUntil, currentGapMs, noteBackfillCount}
window.__qfOrderGetTemplate()                // captured /order/get URL + body
window.__qfTryOrderGet('orderId')            // single backfill call for diagnostics
```

**Safety:**
- `captureNotesFromOrderDetail` patch state.records แบบ immutable copy (per CLAUDE.md rules) — keeps existing note text if patch is empty
- ไม่ block print path: `try { await tryBackfillNotes } catch {}` — fail silently
- Always-on `message` listener (set ที่ module load) จับ prime postMessage จาก iframe หรือ child frame ใดก็ได้
...
| `/api/fulfillment/order/get` (URL random) | POST | Note backfill (buyer/seller remark) | `order_id` + `order_id_list[]` — rate-limited, soft-reject pattern |
